# Development Roadmap — Apliqa V2.0 (Community Edition)
...
V2.0 extends the MVP with three strategic pillars:
...
### User Story (LinkedIn)
> As a job seeker, I want to import my LinkedIn export ZIP so I don't have to upload a PDF CV.
...
| # | Task |
|---|------|
| 9.5 | `apliqa/services/linkedin.py`: parse LinkedIn "Export Data" ZIP → extract `Profile.csv`, `Positions.csv`, `Skills.csv` → assemble raw text |
| 9.6 | `POST /api/profile/import` extended: accepts multipart ZIP upload (detected by content-type / filename) |
| 9.7 | Frontend: second file input accepting `.zip`; label explains LinkedIn export path |
| 9.8 | Tests: LinkedIn ZIP parser with a fixture export; template rendering for both templates |

### Done When
Select "Modern Swiss" → generate → preview renders the correct template. Upload a LinkedIn ZIP → `GET /api/profile` returns a populated `MasterProfile`.
...
| 9 | Second Template + LinkedIn | "Modern Swiss" template; LinkedIn ZIP import |
