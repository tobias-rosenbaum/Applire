...
**Tech stack:** Python 3.12 / FastAPI / PostgreSQL 16 / Next.js 15 / Tailwind / ShadCN. Full details in `CLAUDE.md`.
...
### 3. Check the sprint spec (if working on a sprint)

Sprint specifications live in `Documents/Sprints/` (internal, not committed). If the user provides a sprint spec, read it before planning. It contains acceptance criteria that override generic best practices.
...
├── CLAUDE.md                    # Claude Code-specific agent guide
└── AGENTS.md                    # This file
```
...
Applire is built in sprints. When given a sprint task:

1. **Create a branch**: `git checkout -b sprint-N` (never commit sprint work directly to `main`).
2. **Read the sprint spec** if provided — acceptance criteria are the ground truth.
3. **Plan before coding** — for non-trivial work, write a plan and confirm before implementing.
4. **Reference ADRs** — if a decision has a relevant ADR in `docs/ARCHITECTURE.md`, follow it. If you need to deviate, flag it.
5. **Test as you go** — unit tests for new backend logic, Playwright tests for new user journeys.
6. **After implementation**: all CI tests must pass before the branch is ready for review.
